...
const Insert = async () => {
    ...
    for(let i = 0; i <= hypertubeUsers.length; i++) {
        ...
        const user = {
            ...
            image: `https://avatars.dicebear.com/api/bottts/${Math.round(Math.random() * 100000000000000)}.svg`,
            password: bcrypt.hash('Hypertube42.', 10, function(err, hash) {})
        }
    ...
    await User.collection.insertMany(movieList);
